...
public:
	...
	const void drawRectangle(float x, float y, int b_width, int b_height,short c = 0x2588, float angle = 0, short col = whiteF,int border=0,short bordCol=greenF)
	{
		// float tempx;
		// float tempy;
	...
	const void drawCircle(float xc, float yc, float r, short c = 0x2588,short bordercol = whiteF, bool filled = 0, short innerCol = whiteF)
	{
		...
		if (filled)
		{
			for (float yn = -r; yn < r; yn += 0.1)
				for (float xn = -r; xn < r; xn += 0.1)
					if (xn * xn + yn * yn < r * r)
						Plot(xc + xn, yc + yn, innerCol,c);
		}

		Plot(x + xc, y + yc, bordercol,c);
		float pk = 3 - 2 * r;
		while (y > x)
		{
			...
			if (pk < 0)
			{
				...
				for (auto i : coords1)
					Plot(i.first + xc, i.second + yc, bordercol,c);
			}
			else
			{
				pk += 4 * (x - y) + 10;
